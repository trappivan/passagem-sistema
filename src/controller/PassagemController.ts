...
export async function createPassagem(req: Request, res: Response) {
	const { passageiro_id, linha_id, numero_assento, valor_passagem } = req.body;
...
export async function getAllPassagens(req: Request, res: Response) {
	const passagens = await AppDataSource.getRepository(Passagem).find({
		relations: ["passageiro", "linha_id"],
	});
